<?php
    ...
    or die('Lỗi kết nối');
    //Xử lí button Lưu
    $masp='';$tensp='';$dvt='';$soluongnhap='';
        if(isset($_POST['btnluu'])){
            $masp=$_POST['txtmasp'];
            $tensp=$_POST['txttensp'];
            $dvt=$_POST['txtdvt'];
            $soluongnhap=$_POST['txtsoluongnhap'];
            if($masp==''||$tensp==''||$dvt==''||$soluongnhap==''){
                echo "<script>alert('Phải nhập đủ dữ liệu!')</script>";
            }
            else{
    //Kiểm tra dữ liệu rỗng(mã loại)
        $sql1="SELECT * FROM ql_thongke_nhap WHERE Masp='$masp'";
        $dt=mysqli_query($con,$sql1);
        $sql2="SELECT * FROM ql_thongke_nhap WHERE Tensp='$tensp'";
        ...
        $sql="INSERT INTO ql_thongke_nhap VALUES('$masp','$tensp','$dvt','$soluongnhap')";
        $kq=mysqli_query($con,$sql);
        if($kq) {echo "<script>alert('Thêm mới thành công!')</script>";
            header("location: TKnhapxuat.php");
            exit;
        }
...
<head>
    ...
    <title>Thống kê nhập xuất | Quản lý kho hàng</title>
                ...
                <form method="post" action=""> 
                    <table>
                        <tr>
                            ...
                            <h5 class="ttitle">CẬP NHẬT THỐNG KÊ NHẬP</h5>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="col1">Số lượng nhập</td>
                            <td class="col2">
                                <input class="form-control" type="number" name='txtsoluongnhap' value="<?php echo $soluongnhap ?>">
                            </td>
                        </tr>
                        <tr>
                            <td class="col1"></td>
