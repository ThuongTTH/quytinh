<?php
    ...
    //gán màu nền
    $sheet->getStyle('A'.$rowCount.':C'.$rowCount)->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('00FF00');
    //căn giữa
    $sheet->getStyle('A'.$rowCount.':C'.$rowCount)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    while($row=mysqli_fetch_array($data)){
        $rowCount++;
        $sheet->setCellValue('A'.$rowCount,$row['Mã khách hàng']);
        $sheet->setCellValue('B'.$rowCount,$row['Tên khách hàng']);
        $sheet->setCellValue('C'.$rowCount,$row['Địa chỉ khách hàng']);
        $sheet->setCellValue('D'.$rowCount,$row['Số điện thoại khách hàng']);
        $sheet->setCellValue('E'.$rowCount,$row['Tên sản phẩm']);
        $sheet->setCellValue('F'.$rowCount,$row['Số lượng']);
        $sheet->setCellValue('G'.$rowCount,$row['Đơn giá']);
        $sheet->setCellValue('H'.$rowCount,$row['Tổng giá']);
    }
    //Kẻ bảng 
    $styleAray=array(
        'borders'=>array(
...
<head>
    ...
    <title>Thhống kê xuất hàng | Quản lý kho hàng</title>
